  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const { error: profileError } = await supabase.from("ArtistProfile").insert(profileData);
        if (profileError) {
          console.error("Profile creation error:", profileError);
          // Don't fail registration if profile creation fails
          toast.warning("Account created but profile setup failed. Please complete your profile.");
        }
    ...
    <div className="mx-auto flex w-full max-w-sm flex-col justify-center px-4 py-16 sm:px-6">
      <Card className="border-stone-200 bg-white shadow-none">
        <CardHeader className="space-y-1">
          <CardTitle className="text-center font-serif text-2xl font-normal text-stone-900">Get started</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name" className="text-stone-700">Name</Label>
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} required className="border-stone-200 focus-visible:ring-stone-400" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="border-stone-200 focus-visible:ring-stone-400" />
            </div>
          ...
          <p className="mt-4 text-center text-sm text-stone-600">
            Already have an account? <Link href="/auth/signin" className="text-stone-900 underline underline-offset-4">Sign in</Link>
          </p>
        </CardContent>
      </Card>
    </div>
...
export default function RegisterPage() {
  return (
    <Suspense fallback={
      <div className="mx-auto flex w-full max-w-sm flex-col justify-center px-4 py-16 sm:px-6">
        <div className="h-64 animate-pulse rounded-sm bg-stone-100" />
      </div>
    }>
      <RegisterForm />
    </Suspense>
